...
export default function ProductsPage() {
  ...
  // ✅ NEW: Category change loading state
  ...
  // ✅ UPDATED: Handle category change with immediate feedback
  const handleCategoryChange = useCallback(async (categoryName) => {
    if (categoryName === activeCategory || loadingRef.current) return;
  ...
  // ✅ UPDATED: Memoize category buttons with loading state
  const categoryButtons = useMemo(() =>
    categories.map((category) => {
  ...
  // ✅ FIXED: Only run when category changes
  useEffect(() => {
