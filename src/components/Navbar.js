...
//     { name: 'Wooden Frames', href: '/products/wooden-doors?category=woodenFrame', icon: '🖼️' },
//     { name: 'Safety Doors', href: '/products/wooden-doors?category=safetyDoors', icon: '🔒' },
//     { name: 'Wooden Beds', href: '/products/wooden-doors?category=woodenBed', icon: '🛏️' },
//     { name: 'Wooden Mandir', href: '/products/wooden-doors?category=woodenMandir', icon: '🛕' },
//     { name: 'Wooden Windows', href: '/products/wooden-doors?category=woodenWindow', icon: '🪟' },
//     { name: 'Wooden Art', href: '/products/wooden-doors?category=woodenArt', icon: '🎨' },
//     { name: 'Sofa Chair', href: '/products/wooden-doors?category=sofaChair', icon: '🛋️' }
...
export default function Navbar() {
  ...
  const [isScrolled, setIsScrolled] = useState(false)
  const [isContactPage, setIsContactPage] = useState(false)
  const [loadingCategory, setLoadingCategory] = useState(null) // ✅ CHANGE: sirf ek category ka loading
  const mobileMenuRef = useRef(null)
  const mobileToggleRef = useRef(null)
  const router = useRouter()
  ...
  // ✅ CHANGE: Har category ke liye alag loading
  const handleCategoryClick = (e, href, categoryName) => {
    e.preventDefault();
    setLoadingCategory(categoryName); // Sirf is category ka loading
    closeMobileMenu();
    router.push(href);
  };
  ...
  useEffect(() => {
    ...
    const handleRouteChange = () => {
      setIsContactPage(window.location.pathname === '/contact')
      setLoadingCategory(null) // ✅ Reset loading on route change
    }
              ...
              <li className="dropdown mega-dropdown">
                ...
                <div className="dropdown-menu mega-menu">
                  <div className="mega-menu-content">
                    ...
                    <div className="mega-menu-grid">
                      {menuData.categories.map((item) => {
                        // ✅ CHECK: Sirf is category ka loading
                        const isLoading = loadingCategory === item.name;
                        return (
                          <a 
                            key={item.href} 
                            href={item.href}
                            onClick={(e) => handleCategoryClick(e, item.href, item.name)}
                            className={`mega-menu-item ${isLoading ? 'loading' : ''}`}
                          >
                            <div className="category-icon-box">
                              <span className="item-icon">{item.icon}</span>
                              {isLoading && <span className="nav-loading-spinner">⏳</span>}
                            </div>
                            <div className="category-content">
                              <span className="category-name">{item.name}</span>
                              <span className="category-desc">Premium Quality</span>
                ...
                {menuData.categories.map((item) => {
                  ...
                  return (
                    <a 
                      ...
                      href={item.href}
                      onClick={(e) => handleCategoryClick(e, item.href, item.name)}
                      className={`mobile-category-item ${isLoading ? 'loading' : ''}`}
                    >
                      <div className="mobile-category-icon">
                        <span>{item.icon}</span>
                        {isLoading && <span className="nav-loading-spinner">⏳</span>}
